...
치료가 멈춰 있을 때 사운드 카드를 누르면 `previewSound()`가 그 소리를 약 2초(`previewSeconds`) 들려주고 부드럽게 끕니다. 미리듣기도 노치가 적용된 동일한 신호 경로를 거칩니다. 치료 재생 중에는 기존처럼 바로 소리가 전환됩니다.

### 4. 환경음의 스펙트럼 (TINNITUS_SOUND_PROFILES)
